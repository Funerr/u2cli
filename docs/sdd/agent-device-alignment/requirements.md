...
AndroidTestClii 已具备 Android Agent CLI 的核心子命令树、JSON 契约、错误码、per-serial 串行锁与 snapshot 能力。当前对外推荐入口是 `AndroidTestClii`，历史 `android-cli` 和 `androidtestclii` 命令继续作为兼容入口。现有执行层包含 `uiautomator2`，但项目定位是 AndroidTestClii，不局限于 u2。

在多 Agent 接入中，`agent-device` 风格 CLI 对 Agent 更友好：扁平顶层命令、`@eN` 短引用、snapshot 缓存、session 自动 hydrate、结构化诊断和高频组合命令。本 spec 定义 AndroidTestClii 对齐这些体验的需求。
...
- `REQ-ADA-009`：不破坏 `androidtestclii-core` 中已有 JSON 契约和旧命令行为。
...
**Statement**：新增能力不得破坏 `androidtestclii-core` 中定义的 stdout JSON、stderr、退出码、错误码、artifact 和旧命令兼容性。

**Acceptance**：

- `TEST-ADA-031`：旧命令 `android-cli element click --text 登录 --json`、`androidtestclii element click --text 登录 --json` 行为不变，推荐入口 `AndroidTestClii element click --text 登录 --json` 等价可用。
- `TEST-ADA-032`：所有新增命令 stdout 仍是单个 JSON 对象。
- `TEST-ADA-033`：新增错误码只扩展集合，不改变旧错误码语义。

## Compatibility

- stdout JSON：沿用 `androidtestclii-core` 顶层字段。
- stderr 日志：沿用核心日志约定。
- 退出码：沿用核心退出码。
