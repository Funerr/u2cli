...
- [x] `TASK-ADC-001`：建立 capability registry 和命令到能力层映射
  - Covers：`REQ-ADC-001`
  - Design：`DES-ADC-001`
  - Verification：`TEST-ADC-001`

- [x] `TASK-ADC-002`：为现有核心命令结果补充 `data.metadata.capabilityLayer`
  - Covers：`REQ-ADC-001`
  - Design：`DES-ADC-001`
  - Verification：`TEST-ADC-002`

- [x] `TASK-ADC-003`：补充 fallback/degraded/failureStage/recoveryHint 字段模型
  - Covers：`REQ-ADC-001`、`REQ-ADC-003`
  - Design：`DES-ADC-001`
  - Verification：`TEST-ADC-003`

- [x] `TASK-ADC-004`：新增 `runtime status`、`runtime clear`、`session status`、`session list`
  - Covers：`REQ-ADC-002`
  - Design：`DES-ADC-001`
...
- [x] `TASK-ADC-005`：新增 `snapshot capture --full` 命令和 full snapshot 数据契约
  - Covers：`REQ-ADC-004`
  - Design：`DES-ADC-002`
  - Verification：`TEST-ADC-005`

- [x] `TASK-ADC-006`：为 compact/default snapshot 明确 `canProveAbsence=false`
  - Covers：`REQ-ADC-004`
  - Design：`DES-ADC-002`
  - Verification：`TEST-ADC-006`

- [x] `TASK-ADC-007`：实现 `diff screenshot`，输出 diffRatio、changedPixels、passed 和 overlay artifact
  - Covers：`REQ-ADC-005`
  - Design：`DES-ADC-002`
  - Verification：`TEST-ADC-007`

- [x] `TASK-ADC-008`：实现 `diff snapshot` 节点签名对比
  - Covers：`REQ-ADC-005`
  - Design：`DES-ADC-002`
  - Verification：`TEST-ADC-008`

- [x] `TASK-ADC-009`：实现 `screenshot --overlay-refs`
  - Covers：`REQ-ADC-005`
  - Design：`DES-ADC-002`
...
- [x] `TASK-ADC-010`：实现 `logs start/stop/clear/mark/path/doctor`
  - Covers：`REQ-ADC-006`
  - Design：`DES-ADC-003`
  - Verification：`TEST-ADC-010`

- [x] `TASK-ADC-011`：实现 marker 后日志过滤和 logs artifact
  - Covers：`REQ-ADC-006`
  - Design：`DES-ADC-003`
  - Verification：`TEST-ADC-011`

- [x] `TASK-ADC-012`：实现 `trace start/stop` 和 trace artifact
  - Covers：`REQ-ADC-007`
  - Design：`DES-ADC-003`
  - Verification：`TEST-ADC-012`

- [x] `TASK-ADC-013`：实现 `perf collect` procfs 快照
  - Covers：`REQ-ADC-008`
  - Design：`DES-ADC-003`
  - Verification：`TEST-ADC-013`

- [x] `TASK-ADC-014`：实现顶层 `network` summary，从 logcat/log artifact 提取网络线索
  - Covers：`REQ-ADC-009`
  - Design：`DES-ADC-003`
...
- [x] `TASK-ADC-015`：实现 `settings animations/wifi/airplane/permission` 写入和读回验证
  - Covers：`REQ-ADC-010`
  - Design：`DES-ADC-004`
  - Verification：`TEST-ADC-015`

- [x] `TASK-ADC-016`：实现 `push` broadcast/deep link 封装和结果解析
  - Covers：`REQ-ADC-011`
  - Design：`DES-ADC-004`
  - Verification：`TEST-ADC-016`

- [x] `TASK-ADC-017`：实现 `trigger-app-event` intent 封装和 `am start` 输出解析
  - Covers：`REQ-ADC-011`
  - Design：`DES-ADC-004`
  - Verification：`TEST-ADC-017`

- [x] `TASK-ADC-018`：实现 `boot`、`ensure-simulator` 和 session stale 校验增强
  - Covers：`REQ-ADC-012`
  - Design：`DES-ADC-004`
...
- [x] `TASK-ADC-019`：定义 `.ad` replay 脚本格式和 parser
  - Covers：`REQ-ADC-013`
  - Design：`DES-ADC-005`
  - Verification：`TEST-ADC-019`

- [x] `TASK-ADC-020`：实现 `replay` 串行执行、context/env、变量和引号参数
  - Covers：`REQ-ADC-013`
  - Design：`DES-ADC-005`
  - Verification：`TEST-ADC-020`

- [x] `TASK-ADC-021`：实现 replay selector/ref healing 和 `--replay-update`
  - Covers：`REQ-ADC-014`
  - Design：`DES-ADC-005`
  - Verification：`TEST-ADC-021`

- [x] `TASK-ADC-022`：实现 `test` 批量 replay、失败汇总和 JUnit artifact
  - Covers：`REQ-ADC-015`
  - Design：`DES-ADC-005`
  - Verification：`TEST-ADC-022`

- [x] `TASK-ADC-023`：实现 `# expect-screenshot` 视觉断言
  - Covers：`REQ-ADC-016`
  - Design：`DES-ADC-005`
...
- [x] `TASK-ADC-024`：实现 `gesture pan/fling/replay` 单指 fast path
  - Covers：`REQ-ADC-017`
  - Design：`DES-ADC-006`
  - Verification：`TEST-ADC-024`

- [x] `TASK-ADC-025`：实现 `screen multi-touch/pinch/expand` structured unavailable 和 fallback 建议
  - Covers：`REQ-ADC-017`
  - Design：`DES-ADC-006`
  - Verification：`TEST-ADC-025`

- [x] `TASK-ADC-026`：实现 `gesture record` 占位或录制能力，至少返回 replay JSON 模板
  - Covers：`REQ-ADC-018`
  - Design：`DES-ADC-006`
  - Verification：`TEST-ADC-026`

- [x] `TASK-ADC-027`：实现顶层 `record start/stop` session 化后台录屏
  - Covers：`REQ-ADC-019`
  - Design：`DES-ADC-006`
...
- [x] `TASK-ADC-028`：为 HarmonyOS 相关入口返回稳定 unsupported 或制定独立平台 spec
  - Covers：`REQ-ADC-020`
  - Design：`DES-ADC-007`
  - Verification：`TEST-ADC-028`

- [x] `TASK-ADC-029`：为 React Native、React DevTools、云设备、daemon 等入口返回稳定 unsupported
  - Covers：`REQ-ADC-021`
  - Design：`DES-ADC-007`
  - Verification：`TEST-ADC-029`

- [x] `TASK-ADC-030`：同步 README、Pi schema 和 SDD 追踪矩阵
...
- 新增错误码必须同步 `androidtestclii-core` 或本 spec compatibility。
