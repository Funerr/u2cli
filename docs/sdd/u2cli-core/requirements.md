# AndroidTestClii Core Requirements
...
AndroidTestClii 是面向 Agent / Pi tool 的 Android 自动化 CLI。对外推荐入口是 `AndroidTestClii`；`android-cli` 和 `androidtestclii` 保留为兼容命令名，`androidtestclii` 也继续作为 Python 包名、源码目录名和历史 spec 名。
...
- `TEST-CORE-003`：`AndroidTestClii --help` 可发现核心命令族，兼容入口 `android-cli --help`、`androidtestclii --help` 行为保持可用。
...
- `TEST-CORE-014`：`AndroidTestClii pi schema` 能导出可消费的工具 schema，兼容入口 `android-cli pi schema`、`androidtestclii pi schema` 行为保持可用。
...
- Pi schema：需要随命令契约同步。
- 旧命令兼容：本 spec 固化现有核心子命令树和 `android-cli` / `androidtestclii` 兼容入口，后续不应破坏旧命令行为，除非有新的 approved spec 明确替代。
