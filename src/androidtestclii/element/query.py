...
def find(ctx: CommandContext, selector: Selector | str | dict[str, Any]) -> dict[str, Any]:
    ...
    if resolved.selector is None:
        ...
        return {
            ...
            "matched": True,
            "matchCount": 1,
            "element": resolved.ref.public_dict(),
            "cached": True,
    ...
    def _run() -> dict[str, Any]:
        obj, count = locator_for(device, selector_obj)
...
def exists(ctx: CommandContext, selector: Selector | str | dict[str, Any]) -> dict[str, Any]:
    resolved = resolve_element_target(ctx, selector)
    if resolved.selector is None:
        if resolved.ref is None:
            raise_invalid_ref(resolved.ref_name, resolved.ref)
...
def count(ctx: CommandContext, selector: Selector | str | dict[str, Any]) -> dict[str, Any]:
    resolved = resolve_element_target(ctx, selector)
    if resolved.selector is None:
        if resolved.ref is None:
            raise_invalid_ref(resolved.ref_name, resolved.ref)
        return {"selector": resolved.ref.selector or {}, "matchCount": 1, "cached": True}
    selector_obj = resolved.selector
    assert selector_obj is not None
    device = connect_device(ctx.serial, ctx.timeout_ms)

    def _run() -> dict[str, Any]:
        _, match_count = locator_for(device, selector_obj)
        return {"selector": selector_obj.public_dict(), "matchCount": match_count}

    return run_with_timeout(_run, ctx.timeout_ms)


def bounds(ctx: CommandContext, selector: Selector | str | dict[str, Any]) -> dict[str, Any]:
    resolved = resolve_element_target(ctx, selector)
    if resolved.selector is None:
        if resolved.ref is None:
            raise_invalid_ref(resolved.ref_name, resolved.ref)
    ...
    def _run() -> dict[str, Any]:
        obj, match_count = resolve_unique(device, selector_obj)
...
def wait(ctx: CommandContext, selector: Selector | str | dict[str, Any]) -> dict[str, Any]:
    resolved = resolve_element_target(ctx, selector)
    if resolved.selector is None:
        if resolved.ref is None:
            raise_invalid_ref(resolved.ref_name, resolved.ref)
