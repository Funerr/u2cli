...
from u2cli.device.connect import adb_path, list_adb_devices
from u2cli.element import action as element_action
from u2cli.element import query as element_query
from u2cli.element.selector import Selector, parse_target_selector, selector_from_kwargs
...
from u2cli.screen.snapshot_backend import SnapshotBackendOptions
from u2cli.session.store import SnapshotRef, read_session, ref_entry, update_session
...
def screenshot(ctx: CommandContext, out: str | None = None) -> tuple[dict[str, Any], list[dict[str, Any]]]:
...
def install_from_source(ctx: CommandContext, source: str) -> dict[str, Any]:
    if source.startswith(("http://", "https://")):
        fd, path = tempfile.mkstemp(prefix="u2cli-install-", suffix=".apk")
        os.close(fd)
        try:
...
def click(
    ...
    *,
    ...
    if resolved.selector is None:
        raise_invalid_ref(target, resolved.ref)
    cached = cached_matches(ctx, resolved.selector)
    result: dict[str, Any] | None = None
    for index in range(repetitions):
        try:
            if hold_ms is not None:
                result = element_action.long_click(ctx, resolved.selector)
            else:
                result = element_action.click(ctx, resolved.selector)
        except BaseException as exc:
...
def disconnect(ctx: CommandContext) -> dict[str, Any]:
    from u2cli.session.store import clear_session
...
def resolve_target(ctx: CommandContext, target: str) -> ResolvedTarget:
    if target.startswith("@e"):
        entry, snapshot = ref_entry(target)
    ...
    coords = parse_coordinate_target(target)
    if coords is not None:
        return ResolvedTarget(point=coords)
    return ResolvedTarget(selector=parse_target_selector(target))


def selector_from_target_for_query(target: str) -> Selector:
    if target.startswith("@e"):
        entry, _ = ref_entry(target)
        selector = selector_from_ref(entry)
        if selector is None:
            raise_invalid_ref(target, entry)
        return selector
...
def selector_for_kind(kind: str, value: str) -> Selector:
    ...
    return selector_from_kwargs(**{field: value})
...
def raise_invalid_ref(target: str, ref: SnapshotRef | None) -> None:
    raise U2CliError(
        ...
        {"ref": target, "entry": ref.public_dict() if ref is not None else None},
    )
...
def target_text_summary(snapshot_data: dict[str, Any], target_text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "refs": [f"@{match['ref']}" for match in matches if match.get("ref")],
        "matches": matches,
        "source": source,
