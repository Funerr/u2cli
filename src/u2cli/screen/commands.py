...
from u2cli.screen.snapshot_backend import adb_failure
from u2cli.screen.snapshot_backend import AdbResult
...
def record(ctx: CommandContext, out: str, duration_sec: int = 10) -> dict[str, Any]:
    ...
    with serial_lock(ctx.serial, ctx.timeout_ms):
        def _run() -> dict[str, Any]:
            ...
            remote = "/sdcard/u2cli-record.mp4"
...
def _record_with_screencap(ctx: CommandContext, out: str, duration_sec: int) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="u2cli-record-") as tmp:
        tmp_dir = Path(tmp)
        for index in range(frame_count):
            ...
            if proc.returncode != 0:
                raise U2CliError(
                    ...
                    {"exitCode": proc.returncode, "stderr": proc.stderr.decode(errors="replace")},
                )
