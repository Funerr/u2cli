...
ANDROID_JAR_PROTOCOL = "u2cli-android-snapshot-jar-v1"
ANDROID_JAR_ENTRY_CLASS = "io.github.funerr.u2cli.snapshotjar.SnapshotDump"
ANDROID_JAR_REMOTE_PATH = "/data/local/tmp/u2cli-android-snapshot.jar"
ANDROID_JAR_METADATA_PREFIX = "U2CLI_SNAPSHOT_METADATA_BASE64:"
ANDROID_JAR_XML_CHUNK_PREFIX = "U2CLI_SNAPSHOT_XML_CHUNK:"

ANDROID_HELPER_PACKAGE = "com.callstack.ata.snapshothelper"
ANDROID_HELPER_RUNNER = f"{ANDROID_HELPER_PACKAGE}/.SnapshotInstrumentation"
ANDROID_HELPER_PROTOCOL = "android-snapshot-helper-v1"
        ...
        if artifact:
            ...
            raise U2CliError(
                ...
                "Android snapshot helper APK was not found",
                {
                    ...
                    "env": "U2CLI_ANDROID_SNAPSHOT_HELPER_APK",
                },
            )
        ...
        if snapshot_jar:
            ...
            raise U2CliError(
                ...
                "Android snapshot JAR was not found",
                {
                    ...
                    "env": "U2CLI_ANDROID_SNAPSHOT_JAR",
                },
            )
...
def parse_helper_output(output: str) -> dict[str, Any]:
    ...
    final_result = next(
        (
            ...
            if record.get("ataProtocol") == ANDROID_HELPER_PROTOCOL
        ),
        None,
    )
...
def read_helper_metadata(final_result: dict[str, str]) -> dict[str, Any]:
    metadata: dict[str, Any] = {
        "protocol": final_result.get("ataProtocol"),
...
def resolve_snapshot_helper(explicit_path: str | None) -> SnapshotHelperArtifact | None:
    candidates = [
        explicit_path,
        os.environ.get("U2CLI_ANDROID_SNAPSHOT_HELPER_APK"),
...
def package_snapshot_helper_apk() -> str | None:
    try:
        files = resources.files("u2cli").joinpath("android-snapshot-helper")
...
def resolve_snapshot_jar(explicit_path: str | None) -> str | None:
    candidates = [
        explicit_path,
        os.environ.get("U2CLI_ANDROID_SNAPSHOT_JAR"),
...
def package_snapshot_jar() -> str | None:
    try:
        files = resources.files("u2cli").joinpath("android-snapshot-jar")
    except (ModuleNotFoundError, AttributeError):
        return None
    try:
